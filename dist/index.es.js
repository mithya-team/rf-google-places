import { MUIReadOnly, attachField } from 'react-forms';
import React, { useState, useEffect } from 'react';
import { TextField, InputAdornment, IconButton, ListItemText } from '@material-ui/core';
...
var GoogleUtils = {
    ...
    /* utils functions */
    placeSuggest: function (input, types) {
        return (new Promise(function (resolve) {
            if (google) {
                var request = { input: input, types: types };
                var service = new google.maps.places.AutocompleteService();
                service.getPlacePredictions(request, function (results) {
                    resolve(results);
                });
            }
        }));
    ...
    placeTypesParser: function (types, typeMap) {
        if (!types)
            return [];
        var newTypes = [];
        var keys = Object.keys(typeMap);
        keys.forEach(function (item) {
            var values = typeMap[item];
    ...
    transformAddress: function (place, description) {
        ...
        GoogleUtils.geoAddressFields.forEach(function (addressField) {
            addressComponents.forEach(function (addComp) {
    ...
    formatOpeningHours: function (openingHours) {
        ...
        if (!openingHours)
            return undefined;
        var periods = [];
        (_a = openingHours === null || openingHours === void 0 ? void 0 : openingHours.periods) === null || _a === void 0 ? void 0 : _a.forEach(function (period) {
            var p = {
        ...
        React.createElement(Autocomplete, { getOptionLabel: function (option) { return option.description || ''; }, classes: { popper: classes.popper }, filterOptions: function (x) { return x; }, options: result, includeInputInList: true, fullWidth: textFieldProps.fullWidth, autoComplete: true, open: open && input.length > 0, forcePopupIcon: false, disableClearable: !value, getOptionSelected: function (option, value) { return option.description === value.description; }, value: value || null, renderInput: function (params) { return (React.createElement(TextField, __assign({}, params, { onChange: handleInputChange, value: input || '', 
                ...
                InputProps: __assign(__assign({ endAdornment: (React.createElement(InputAdornment, { position: 'end' },
                        ...
                        React.createElement(IconButton, { disableRipple: true, disableFocusRipple: true, disableTouchRipple: true },
                            React.createElement(SearchIcon, { fontSize: "small" })))) }, params.InputProps), textFieldProps.InputProps), placeholder: "Search on google", variant: "standard" }, textFieldProps))); }, renderOption: function (item) { return React.createElement(ListItemText, { onClick: function () { return handleResultClick(item); } }, item.description); } })));
};
